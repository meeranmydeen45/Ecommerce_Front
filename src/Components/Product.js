     ...
     changeSizeIndex(productId, sizeIndex, indexArray)  
        ...
        const productList = items.map((item, i) => {
            return (
             ...
             return (
              ...
              <option value={index +"-"+sizeitem.productId}>{sizeitem.size}</option>
             )
                 ...
                 <div>
                 Quantity: {item.listOfstocksBySize[indexArray[item.id-1].SizeIndex].quantity}
                </div>
                </div>
                <div className="card-footer">
                Cost ${item.listOfstocksBySize[indexArray[item.id-1].SizeIndex].cost}
                <button className="btn btn-primary" onClick = {() => this.props.addToCart(item, indexArray, cartItems)}>Add</button>
                </div>
                </div>
                </div>
        ...
        return(
            <div className="div-Product">
               <div style ={{position:"absolute", textAlign:"center", color:"gold", fontWeight:"bold", top:"-40px", right:"-350px"}}>
               <NavLink to="/cardList" className="btn btn-secondary">
        <span class="fa fa-shopping-cart">{totalProducts}</span></NavLink> 
               </div>
